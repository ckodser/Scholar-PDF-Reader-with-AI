 ...
 * This file contains the shared logic for both the in-page and pop-out chat.
 */
...
function cacheDomElements() {
    ...
    dom.popoutBtn = document.getElementById('ai-chat-popout-btn');
}
...
async function handleSendMessage() {
    ...
    const activeTab = aiChatState.tabs.find(t => t.id === aiChatState.activeTabId);
    if (!userInput || !activeTab || activeTab.isThinking) return;

    let thinkingIndicator;
    try {
        renderMessage('user', userInput);

        // Explicitly ensure PDF data is loaded for the first message in a tab.
        // This is crucial for the popout, where the fetch might not have completed on initial load.
        if (activeTab.conversation.length === 0 && !aiChatState.pdfDataB64) {
            console.log("First message in tab, ensuring PDF is loaded...");
            await preparePdfForConversation();
            if (!aiChatState.pdfDataB64) {
                renderMessage('ai', 'Error: Could not load the PDF for this chat. Please check your connection and permissions.');
                return;
            }
        }
        ...
        thinkingIndicator = await showThinkingIndicator();
        activeTab.thinkingIndicator = thinkingIndicator;
        ...
        if (aiChatState.activeTabId === activeTab.id && thinkingIndicator) {
        ...
        if (activeTab) {
            ...
            activeTab.thinkingIndicator = null; // Clear the reference
            if (aiChatState.activeTabId === activeTab.id) {
                dom.chatSendBtn.disabled = false;
            }
        }
